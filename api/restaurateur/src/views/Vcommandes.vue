<template>
  <div class="sidebar-layout">
    ...
    <div class="card">
  <h1 class="title">Visualiser les commandes</h1>
...
export default defineComponent({
  ...
  methods: {
    async fetchMenus() {
      ...
      try {
        const response = await axios.get('http://localhost/app/orders/', {
          headers: {
    ...
    async addMenu() {
      const accessToken = this.$cookies.get('accessToken');
      try {
        const response = await axios.post('http://localhost/app/orders/', this.newItem, {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
        });
